# Project mission is to provide easy to use, powerful, C (C/C++) embeddable web server with optional CGI, SSL and Lua support.
# https://github.com/civetweb/civetweb/

include_guard(DIRECTORY)

# =========== third party civetweb ==================
function(PROJECT_THIRD_PARTY_CIVETWEB_PATCH_IMPORTED_TARGET TARGET_NAME)
  unset(PATCH_REMOVE_RULES)
  unset(PATCH_ADD_TARGETS)
  if(TARGET OpenSSL::SSL
     OR TARGET OpenSSL::Crypto
     OR TARGET LibreSSL::TLS)
    list(APPEND PATCH_REMOVE_RULES "(lib)?crypto" "(lib)?ssl")
    list(APPEND PATCH_ADD_TARGETS ${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CRYPT_LINK_NAME})
  endif()

  if(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_LUA_LINK_NAMES)
    list(APPEND PATCH_REMOVE_RULES "(lib)?lua(_a)?")
    list(APPEND PATCH_ADD_TARGETS ${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_LUA_LINK_NAME})
  endif()

  if(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_SNAPPY_LINK_NAME)
    list(APPEND PATCH_REMOVE_RULES "(lib)?snappy\\.(so|a|dll|lib|dylib)\$")
    list(APPEND PATCH_ADD_TARGETS ${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_SNAPPY_LINK_NAME})
  endif()

  if(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZSTD_LINK_NAME)
    list(APPEND PATCH_REMOVE_RULES "(lib)?zstd\\.(so|a|dll|lib|dylib)\$")
    list(APPEND PATCH_ADD_TARGETS ${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZSTD_LINK_NAME})
  endif()

  if(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_NAME)
    list(APPEND PATCH_REMOVE_RULES "(lib)?z\\.(so|a|dll|lib|dylib)\$")
    list(APPEND PATCH_ADD_TARGETS ${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_NAME})
  endif()

  if(PATCH_REMOVE_RULES OR PATCH_ADD_TARGETS)
    project_build_tools_patch_imported_link_interface_libraries(${TARGET_NAME} REMOVE_LIBRARIES ${PATCH_REMOVE_RULES}
                                                                ADD_LIBRARIES ${PATCH_ADD_TARGETS})
  endif()
  project_build_tools_patch_default_imported_config(${TARGET_NAME})
endfunction()

macro(PROJECT_THIRD_PARTY_CIVETWEB_IMPORT)
  if(TARGET civetweb::civetweb-cpp)
    message(STATUS "Dependency(${PROJECT_NAME}): civetweb found target civetweb::civetweb-cpp")
    set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_LINK_NAME civetweb::civetweb-cpp)
  elseif(TARGET civetweb::civetweb)
    message(STATUS "Dependency(${PROJECT_NAME}): civetweb found target civetweb::civetweb")
    set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_LINK_NAME civetweb::civetweb)
  endif()

  if(TARGET civetweb::civetweb-cpp)
    project_third_party_civetweb_patch_imported_target(civetweb::civetweb-cpp)
  endif()
  if(TARGET civetweb::civetweb)
    project_third_party_civetweb_patch_imported_target(civetweb::civetweb)
  endif()
endmacro()

if(NOT TARGET civetweb::civetweb-cpp AND NOT TARGET civetweb::civetweb)
  # civetweb may use librt,libdl and WinSock from system
  if(CMAKE_FIND_ROOT_PATH_MODE_LIBRARY AND NOT CMAKE_FIND_ROOT_PATH_MODE_LIBRARY STREQUAL "BOTH")
    set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BACKUP_CMAKE_FIND_ROOT_PATH_MODE_LIBRARY
        ${CMAKE_FIND_ROOT_PATH_MODE_LIBRARY})
    set(CMAKE_FIND_ROOT_PATH_MODE_LIBRARY "BOTH")
  else()
    unset(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BACKUP_CMAKE_FIND_ROOT_PATH_MODE_LIBRARY)
  endif()
  if(CMAKE_FIND_ROOT_PATH_MODE_INCLUDE AND NOT CMAKE_FIND_ROOT_PATH_MODE_INCLUDE STREQUAL "BOTH")
    set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BACKUP_CMAKE_FIND_ROOT_PATH_MODE_INCLUDE
        ${CMAKE_FIND_ROOT_PATH_MODE_INCLUDE})
    set(CMAKE_FIND_ROOT_PATH_MODE_INCLUDE "BOTH")
  else()
    unset(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BACKUP_CMAKE_FIND_ROOT_PATH_MODE_INCLUDE)
  endif()
  if(CMAKE_FIND_ROOT_PATH_MODE_PACKAGE AND NOT CMAKE_FIND_ROOT_PATH_MODE_PACKAGE STREQUAL "BOTH")
    set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BACKUP_CMAKE_FIND_ROOT_PATH_MODE_PACKAGE
        ${CMAKE_FIND_ROOT_PATH_MODE_PACKAGE})
    set(CMAKE_FIND_ROOT_PATH_MODE_PACKAGE "BOTH")
  else()
    unset(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BACKUP_CMAKE_FIND_ROOT_PATH_MODE_PACKAGE)
  endif()
  find_package(civetweb QUIET CONFIG)
  project_third_party_civetweb_import()
  if(NOT TARGET civetweb::civetweb-cpp AND NOT TARGET civetweb::civetweb)
    if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS)
      set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS
          "-DCMAKE_POSITION_INDEPENDENT_CODE=ON" "-DCIVETWEB_BUILD_TESTING=OFF" "-DCIVETWEB_ENABLE_DEBUG_TOOLS=OFF"
          "-DCIVETWEB_ENABLE_CXX=ON" "-DCIVETWEB_ENABLE_IPV6=ON" "-DCIVETWEB_ENABLE_WEBSOCKETS=ON")
      if(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_THREAD_STACK_SIZE)
        list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS
             "-DCIVETWEB_THREAD_STACK_SIZE=${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_THREAD_STACK_SIZE}")
      endif()
      if(PROJECT_COMPILER_OPTIONS_TARGET_USE_SANITIZER STREQUAL "address")
        list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS "-DCIVETWEB_ENABLE_ASAN=ON")
      else()
        list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS "-DCIVETWEB_ENABLE_ASAN=OFF")
      endif()

      if(CMAKE_CROSSCOMPILING)
        list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS
             "-DCIVETWEB_ENABLE_SERVER_EXECUTABLE=OFF")
      endif()
      if(TARGET ZLIB::ZLIB)
        list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS "-DCIVETWEB_ENABLE_ZLIB=ON")
      endif()
      if((OPENSSL_FOUND OR OpenSSL_FOUND) AND NOT LIBRESSL_FOUND)
        list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS "-DCIVETWEB_ENABLE_SSL=ON"
             "-DCIVETWEB_ENABLE_SSL_DYNAMIC_LOADING=OFF")
        if((OPENSSL_VERSION AND OPENSSL_VERSION VERSION_GREATER_EQUAL "3.0.0")
           OR (OpenSSL_VERSION AND OpenSSL_VERSION VERSION_GREATER_EQUAL "3.0.0"))
          list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS "-DCIVETWEB_SSL_OPENSSL_API_1_1=OFF"
               "-DCIVETWEB_SSL_OPENSSL_API_3_0=ON")
        elseif((OPENSSL_VERSION AND OPENSSL_VERSION VERSION_LESS "1.1") OR (OpenSSL_VERSION AND OpenSSL_VERSION
                                                                                                VERSION_LESS "1.1"))
          list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS "-DCIVETWEB_SSL_OPENSSL_API_1_1=OFF"
               "-DCIVETWEB_SSL_OPENSSL_API_1_0=ON")
        endif()
        if(OPENSSL_ROOT_DIR)
          list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS
               "-DOPENSSL_ROOT_DIR=${OPENSSL_ROOT_DIR}" "-DOpenSSL_ROOT=${OPENSSL_ROOT_DIR}")
        endif()
        #[[
        if(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CI_MODE)
          list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS "--debug-find-pkg=OpenSSL")
        endif()
        ]]
      else()
        list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS "-DCIVETWEB_ENABLE_SSL=OFF")
      endif()
      # TODO CIVETWEB_ENABLE_LUA,CIVETWEB_ENABLE_LUA_SHARED
      list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS "-DCIVETWEB_ENABLE_LUA=OFF")

      if(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_APPEND_DEFAULT_BUILD_OPTIONS)
        list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS
             ${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_APPEND_DEFAULT_BUILD_OPTIONS})
      endif()
    endif()
    project_third_party_port_declare(civetweb VERSION "v1.16" GIT_URL "https://github.com/civetweb/civetweb.git")

    project_third_party_append_build_shared_lib_var(
      "civetweb" "" ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS BUILD_SHARED_LIBS)

    # Patch file
    project_third_party_try_patch_file(
      ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_PATCH_FILE "${CMAKE_CURRENT_LIST_DIR}" "civetweb"
      "${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_VERSION}")

    if(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_PATCH_FILE
       AND EXISTS "${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_PATCH_FILE}")
      list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS GIT_PATCH_FILES
           ${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_PATCH_FILE})
    endif()

    find_configure_package(
      PACKAGE
      civetweb
      FIND_PACKAGE_FLAGS
      CONFIG
      BUILD_WITH_CMAKE
      CMAKE_INHERIT_BUILD_ENV
      CMAKE_INHERIT_FIND_ROOT_PATH
      CMAKE_INHERIT_SYSTEM_LINKS
      CMAKE_FLAGS
      ${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_OPTIONS}
      WORKING_DIRECTORY
      "${PROJECT_THIRD_PARTY_PACKAGE_DIR}"
      BUILD_DIRECTORY
      "${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_DIR}"
      PREFIX_DIRECTORY
      "${PROJECT_THIRD_PARTY_INSTALL_DIR}"
      SRC_DIRECTORY_NAME
      "civetweb-${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_VERSION}"
      GIT_BRANCH
      "${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_VERSION}"
      GIT_URL
      "${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_GIT_URL}")

    project_third_party_civetweb_import()
  endif()

  # Restore CMAKE_FIND_ROOT_PATH_MODE_LIBRARY,CMAKE_FIND_ROOT_PATH_MODE_INCLUDE,CMAKE_FIND_ROOT_PATH_MODE_PACKAGE
  if(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BACKUP_CMAKE_FIND_ROOT_PATH_MODE_LIBRARY)
    set(CMAKE_FIND_ROOT_PATH_MODE_LIBRARY
        ${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BACKUP_CMAKE_FIND_ROOT_PATH_MODE_LIBRARY})
    unset(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BACKUP_CMAKE_FIND_ROOT_PATH_MODE_LIBRARY)
  endif()
  if(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BACKUP_CMAKE_FIND_ROOT_PATH_MODE_INCLUDE)
    set(CMAKE_FIND_ROOT_PATH_MODE_INCLUDE
        ${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BACKUP_CMAKE_FIND_ROOT_PATH_MODE_INCLUDE})
    unset(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BACKUP_CMAKE_FIND_ROOT_PATH_MODE_INCLUDE)
  endif()
  if(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BACKUP_CMAKE_FIND_ROOT_PATH_MODE_PACKAGE)
    set(CMAKE_FIND_ROOT_PATH_MODE_PACKAGE
        ${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BACKUP_CMAKE_FIND_ROOT_PATH_MODE_PACKAGE})
    unset(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BACKUP_CMAKE_FIND_ROOT_PATH_MODE_PACKAGE)
  endif()
else()
  project_third_party_civetweb_import()
endif()

if(NOT TARGET civetweb::civetweb-cpp AND NOT TARGET civetweb::civetweb)
  if(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CI_MODE)
    project_build_tools_print_configure_log("${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CIVETWEB_BUILD_DIR}")
  endif()
  message(FATAL_ERROR "-- Dependency(${PROJECT_NAME}): civetweb not found")
endif()
