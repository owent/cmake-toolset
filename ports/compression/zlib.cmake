include_guard(DIRECTORY)

# =========== third party zlib ==================
# force to use prebuilt when using mingw
macro(PROJECT_THIRD_PARTY_ZLIB_IMPORT)
  if(TARGET ZLIB::ZLIB)
    # find static library first
    set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_NAME ZLIB::ZLIB)
    if(NOT ZLIB_LIBRARIES)
      project_build_tools_get_imported_property(ZLIB_LIBRARIES ZLIB::ZLIB IMPORTED_IMPLIB)
      if(NOT ZLIB_LIBRARIES)
        project_build_tools_get_imported_location(ZLIB_LIBRARIES ZLIB::ZLIB IMPORTED_LOCATION)
      endif()
    endif()
    # ZLIB::ZLIB may be an INTERFACE IMPORTED target wrapping ZLIB::ZLIBSTATIC (zlib v1.3.2+)
    if(NOT ZLIB_LIBRARIES AND TARGET ZLIB::ZLIBSTATIC)
      project_build_tools_get_imported_location(ZLIB_LIBRARIES ZLIB::ZLIBSTATIC IMPORTED_LOCATION)
    endif()
    message(STATUS "Dependency(${PROJECT_NAME}): zlib found.(${ZLIB_LIBRARIES})")

    set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_MODE FALSE)
    set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_SELECT_NAME)
    foreach(LIB_NAME ${ZLIB_LIBRARIES})
      if(LIB_NAME MATCHES "optimized|debug|general")
        set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_MODE "${LIB_NAME}")
      elseif(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_MODE STREQUAL "optimized")
        set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME "${LIB_NAME}")
        set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_MODE FALSE)
      elseif(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_MODE STREQUAL "debug")
        set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_DEBUG_NAME "${LIB_NAME}")
        set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_MODE FALSE)
      else()
        set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_SELECT_NAME "${LIB_NAME}")
      endif()
    endforeach()

    if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME)
      get_target_property(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME ZLIB::ZLIB
                          IMPORTED_IMPLIB_RELEASE)
      if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME)
        get_target_property(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME ZLIB::ZLIB
                            IMPORTED_LOCATION_RELEASE)
      endif()
      if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME)
        get_target_property(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME ZLIB::ZLIB
                            IMPORTED_IMPLIB_RELWITHDEBINFO)
      endif()
      if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME)
        get_target_property(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME ZLIB::ZLIB
                            IMPORTED_LOCATION_RELWITHDEBINFO)
      endif()
      if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME AND TARGET ZLIB::ZLIBSTATIC)
        get_target_property(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME ZLIB::ZLIBSTATIC
                            IMPORTED_LOCATION_RELEASE)
        if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME)
          get_target_property(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME ZLIB::ZLIBSTATIC
                              IMPORTED_LOCATION_RELEASE)
        endif()
        if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME)
          get_target_property(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME ZLIB::ZLIBSTATIC
                              IMPORTED_IMPLIB_RELWITHDEBINFO)
        endif()
        if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME)
          get_target_property(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME ZLIB::ZLIBSTATIC
                              IMPORTED_LOCATION_RELWITHDEBINFO)
        endif()
      endif()
    endif()

    if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_DEBUG_NAME)
      get_target_property(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_DEBUG_NAME ZLIB::ZLIB IMPORTED_IMPLIB_DEBUG)
      if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_DEBUG_NAME)
        get_target_property(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_DEBUG_NAME ZLIB::ZLIB
                            IMPORTED_LOCATION_DEBUG)
      endif()
      if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_DEBUG_NAME AND TARGET ZLIB::ZLIBSTATIC)
        get_target_property(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_DEBUG_NAME ZLIB::ZLIBSTATIC
                            IMPORTED_LOCATION_DEBUG)
        if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_DEBUG_NAME)
          get_target_property(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_DEBUG_NAME ZLIB::ZLIBSTATIC
                              IMPORTED_LOCATION_DEBUG)
        endif()
      endif()
    endif()

    if(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME)
      set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_SELECT_NAME
          "${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_OPTIMIZED_NAME}")
    elseif(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_SELECT_NAME
           AND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_DEBUG_NAME)
      set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_SELECT_NAME
          "${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_ZLIB_LINK_DEBUG_NAME}")
    endif()

    if(ZLIB_INCLUDE_DIRS)
      get_filename_component(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_ROOT_DIR "${ZLIB_INCLUDE_DIRS}"
                             DIRECTORY CACHE)
    else()
      get_target_property(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_INCLUDE_DIR ZLIB::ZLIB
                          INTERFACE_INCLUDE_DIRECTORIES)
      # ZLIB::ZLIB may be an INTERFACE IMPORTED target; check ZLIB::ZLIBSTATIC for include dirs
      if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_INCLUDE_DIR AND TARGET ZLIB::ZLIBSTATIC)
        get_target_property(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_INCLUDE_DIR ZLIB::ZLIBSTATIC
                            INTERFACE_INCLUDE_DIRECTORIES)
      endif()
      if(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_INCLUDE_DIR)
        set(ZLIB_INCLUDE_DIRS "${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_INCLUDE_DIR}")
        get_filename_component(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_ROOT_DIR
                               "${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_INCLUDE_DIR}" DIRECTORY CACHE)
      endif()
      unset(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_INCLUDE_DIR)
    endif()
    mark_as_advanced(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_ROOT_DIR)
  endif()
endmacro()

if(NOT TARGET ZLIB::ZLIB)
  project_third_party_port_declare(
    zlib
    PORT_PREFIX
    "COMPRESSION"
    VERSION
    "v1.3.2"
    GIT_URL
    "https://github.com/madler/zlib.git"
    BUILD_OPTIONS
    "-DCMAKE_POSITION_INDEPENDENT_CODE=ON"
    "-DBUILD_TESTING=OFF")

  project_third_party_append_build_shared_lib_var(
    "zlib" "COMPRESSION" ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_BUILD_OPTIONS BUILD_SHARED_LIBS)

  project_third_party_try_patch_file(
    ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_PATCH_FILE "${CMAKE_CURRENT_LIST_DIR}" "zlib"
    "${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_VERSION}")

  if(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_PATCH_FILE
     AND EXISTS "${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_PATCH_FILE}")
    list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_BUILD_OPTIONS GIT_PATCH_FILES
         "${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_PATCH_FILE}")
  endif()

  set(ZLIB_ROOT ${PROJECT_THIRD_PARTY_INSTALL_DIR})

  # zlib do not support c2x now
  if(CMAKE_C_STANDARD AND CMAKE_C_STANDARD GREATER 17)
    set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_BACKUP_CMAKE_C_STANDARD ${CMAKE_C_STANDARD})
    set(CMAKE_C_STANDARD 17)
  endif()
  find_configure_package(
    PACKAGE
    ZLIB
    PORT_PREFIX
    "COMPRESSION"
    BUILD_WITH_CMAKE
    CMAKE_INHERIT_BUILD_ENV
    CMAKE_INHERIT_BUILD_ENV_DISABLE_CXX_FLAGS
    CMAKE_FLAGS
    ${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_BUILD_OPTIONS}
    WORKING_DIRECTORY
    "${PROJECT_THIRD_PARTY_PACKAGE_DIR}"
    PREFIX_DIRECTORY
    "${PROJECT_THIRD_PARTY_INSTALL_DIR}"
    SRC_DIRECTORY_NAME
    "zlib-${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_VERSION}"
    BUILD_DIRECTORY
    "${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_BUILD_DIR}"
    GIT_BRANCH
    "${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_VERSION}"
    GIT_URL
    "${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_GIT_URL}")
  if(DEFINED ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_BACKUP_CMAKE_C_STANDARD)
    set(CMAKE_C_STANDARD ${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_BACKUP_CMAKE_C_STANDARD})
    unset(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_BACKUP_CMAKE_C_STANDARD)
  endif()

  if(NOT TARGET ZLIB::ZLIB)
    echowithcolor(COLOR RED "-- Dependency(${PROJECT_NAME}): zlib is required")
    message(FATAL_ERROR "zlib not found")
  endif()
  project_third_party_zlib_import()
else()
  project_third_party_zlib_import()
endif()

if(NOT TARGET ZLIB::ZLIB)
  if(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CI_MODE)
    project_build_tools_print_configure_log("${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_COMPRESSION_ZLIB_BUILD_DIR}")
  endif()
  message(FATAL_ERROR "Dependency(${PROJECT_NAME}): Can not build zlib.")
endif()
