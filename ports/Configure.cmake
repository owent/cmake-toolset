...
function(project_third_party_try_patch_file OUTPUT_VAR BASE_DIRECTORY PORT_PREFIX VERSION)
  if(CMAKE_CROSSCOMPILING)
    if(EXISTS "${BASE_DIRECTORY}/${PORT_PREFIX}-${VERSION}.cross.patch")
      set(${OUTPUT_VAR}
          "${BASE_DIRECTORY}/${PORT_PREFIX}-${VERSION}.cross.patch"
          PARENT_SCOPE)
      return()
    elseif(VERSION MATCHES "(.*)\\.[^\\.]*$")
      unset(SELECT_PATCH_FILE)
      unset(SELECT_PATCH_VERSION)
      file(GLOB TRY_PATCH_FILES "${BASE_DIRECTORY}/${PORT_PREFIX}-${CMAKE_MATCH_0}*.cross.patch")
      foreach(PATCH_FILE ${TRY_PATCH_FILES})
        get_filename_component(PATCH_FILE_BASE_NAME "${PATCH_FILE}" NAME)
        if(PATCH_FILE_BASE_NAME MATCHES "^${PORT_PREFIX}-(v)?([0-9]+\\..*)\\.cross\\.patch")
          if(NOT SELECT_PATCH_VERSION)
            set(SELECT_PATCH_FILE "${PATCH_FILE}")
            set(SELECT_PATCH_VERSION "${CMAKE_MATCH_2}")
          elseif(CMAKE_MATCH_2 VERSION_GREATER SELECT_PATCH_VERSION)
            set(SELECT_PATCH_FILE "${PATCH_FILE}")
            set(SELECT_PATCH_VERSION "${CMAKE_MATCH_2}")
          endif()
        endif()
  ...
  if(EXISTS "${BASE_DIRECTORY}/${PORT_PREFIX}-${VERSION}.patch")
    set(${OUTPUT_VAR}
        "${BASE_DIRECTORY}/${PORT_PREFIX}-${VERSION}.patch"
        PARENT_SCOPE)
  elseif(VERSION MATCHES "(.*)\\.[^\\.]*$")
    unset(SELECT_PATCH_FILE)
    unset(SELECT_PATCH_VERSION)
    file(GLOB TRY_PATCH_FILES "${BASE_DIRECTORY}/${PORT_PREFIX}-${CMAKE_MATCH_0}*.patch")
    foreach(PATCH_FILE ${TRY_PATCH_FILES})
      get_filename_component(PATCH_FILE_BASE_NAME "${PATCH_FILE}" NAME)
      if(PATCH_FILE_BASE_NAME MATCHES "^${PORT_PREFIX}-(v)?([0-9]+\\..*)\\.patch")
        if(NOT SELECT_PATCH_VERSION)
          set(SELECT_PATCH_FILE "${PATCH_FILE}")
          set(SELECT_PATCH_VERSION "${CMAKE_MATCH_2}")
        elseif(CMAKE_MATCH_2 VERSION_GREATER SELECT_PATCH_VERSION)
