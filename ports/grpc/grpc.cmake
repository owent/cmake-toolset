...
set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_PREBUILT_USABLE FALSE)
if(TARGET gRPC::grpc++_alts
   OR TARGET gRPC::grpc++
   OR TARGET gRPC::grpc)
  if(NOT CMAKE_CROSSCOMPILING OR ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_CPP_PLUGIN_EXECUTABLE)
    set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_PREBUILT_USABLE TRUE)
  elseif(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CI_MODE)
    message(
      STATUS
        "Dependency(${PROJECT_NAME}): gRPC prebuilt packages found, but the host plugins(grpc_cpp_plugin, etc.) are missing. Falling back to the source build."
    )
  endif()
endif()

if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_PREBUILT_USABLE)
  ...
  if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_GRPC_VERSION)
    ...
    if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_GRPC_VERSION)
      if(absl_FOUND AND absl_VERSION VERSION_GREATER_EQUAL "20230125")
        ...
        if(CMAKE_CXX_COMPILER_ID STREQUAL "AppleClang" AND CMAKE_CXX_COMPILER_VERSION VERSION_LESS "15")
          set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_GRPC_VERSION "v1.68.2")
        else()
          set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_GRPC_VERSION "v1.84.0")
        endif()
      else()
  ...
  if(NOT ATFRAMEWORK_CMAKE_TOOLSET_HOST_BUILDING AND CMAKE_CROSSCOMPILING)
    ...
    foreach(CMD_ARG IN LISTS ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_HOST_BUILD_FLAGS)
      ...
      project_build_tools_append_space_one_flag_to_var(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_HOST_BUILD_FLAGS_PWSH
      ...
      project_build_tools_append_space_one_flag_to_var(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_HOST_BUILD_FLAGS_BASH
                                                       "\"${CMD_ARG_BASH_UNESCAPE}\"")
    endforeach()
    unset(CMD_ARG_UNESCAPE)

    # Build host
    if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_HOST_BUILD_CONFIG)
      # The host protobuf is installed with the Release and Debug configurations only(no RelWithDebInfo), and the value
      # of CMAKE_BUILD_TYPE may be changed by other ports during configuring. So we use a fixed matching configuration
      # for the host plugins to avoid mixing the CRT runtimes.
      if(NOT CMAKE_BUILD_TYPE OR CMAKE_BUILD_TYPE STREQUAL "Debug")
        set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_HOST_BUILD_CONFIG "Release")
      else()
        set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_HOST_BUILD_CONFIG "${CMAKE_BUILD_TYPE}")
        if(NOT CMAKE_BUILD_TYPE STREQUAL "Release")
          list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_HOST_BUILD_FLAGS
               "-DCMAKE_MAP_IMPORTED_CONFIG_RELWITHDEBINFO=${CMAKE_BUILD_TYPE}"
               "-DCMAKE_MAP_IMPORTED_CONFIG_MINSIZEREL=${CMAKE_BUILD_TYPE}")
        else()
          list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_HOST_BUILD_FLAGS
               "-DCMAKE_MAP_IMPORTED_CONFIG_RELEASE=${CMAKE_BUILD_TYPE}")
        endif()
      endif()
    endif()
    # The host project defaults to RelWithDebInfo when CMAKE_BUILD_TYPE is empty, and its internal ports would be built
    # in that configuration. Keep the whole host project in the same configuration as the plugins above.
    list(APPEND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_HOST_BUILD_FLAGS
         "-DCMAKE_BUILD_TYPE=${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_HOST_BUILD_CONFIG}")
    ...
    if(NOT ATFRAMEWORK_CMAKE_TOOLSET_PWSH
      ...
      configure_file(
        ...
        "${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_HOST_TOOL_BUILD_DIR}/run-build-host.ps1" @ONLY NEWLINE_STYLE CRLF)
      configure_file(
        "${CMAKE_CURRENT_LIST_DIR}/crosscompiling-grpc-host/run-build-host.bat.in"
        "${ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_GRPC_HOST_TOOL_BUILD_DIR}/run-build-host.bat" @ONLY NEWLINE_STYLE CRLF)

      # build
      execute_process(
        COMMAND "${ATFRAMEWORK_CMAKE_TOOLSET_PWSH}" -NoProfile -InputFormat None -ExecutionPolicy Bypass -NonInteractive
  ...
  if((CMAKE_CROSSCOMPILING OR ATFRAMEWORK_CMAKE_TOOLSET_HOST_BUILDING)
     AND CMAKE_HOST_WIN32
     AND gRPC_MSVC_CONFIGURE STREQUAL "RelWithDebInfo")
    # The host protobuf is installed with the Release and Debug configurations only. RelWithDebInfo is not one of them,
    # and importing from the Debug libraries would mix the CRT runtimes and fail to link. Use Release for the whole host
    # gRPC build(including the plugins) instead.
    set(gRPC_MSVC_CONFIGURE "Release")
  endif()
  if(CMAKE_CROSSCOMPILING)
  ...
  if(CMAKE_CXX_STANDARD)
    if(CMAKE_CXX_STANDARD GREATER 20)
      set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CURRENT_PORT_MAX_CXX_STANDARD 20)
    else()
      set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CURRENT_PORT_MAX_CXX_STANDARD ${CMAKE_CXX_STANDARD})
    endif()
    if(MSVC AND ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CURRENT_PORT_MAX_CXX_STANDARD GREATER 17)
      set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CURRENT_PORT_MAX_CXX_STANDARD 17)
    endif()
  elseif(MSVC)
    if(COMPILER_OPTION_RECOMMEND_CXX_STANDARD GREATER 17)
      set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CURRENT_PORT_MAX_CXX_STANDARD 17)
    else()
      set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CURRENT_PORT_MAX_CXX_STANDARD ${COMPILER_OPTION_RECOMMEND_CXX_STANDARD})
    endif()
  else()
    if(COMPILER_OPTION_RECOMMEND_CXX_STANDARD GREATER 20)
      set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CURRENT_PORT_MAX_CXX_STANDARD 20)
    else()
      set(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_CURRENT_PORT_MAX_CXX_STANDARD ${COMPILER_OPTION_RECOMMEND_CXX_STANDARD})
    endif()
