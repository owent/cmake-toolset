...
macro(project_build_tools_append_cmake_inherit_options OUTVAR)
  cmake_parse_arguments(
    ...
    "DISABLE_C_FLAGS;DISABLE_CXX_FLAGS;DISABLE_ASM_FLAGS;DISABLE_TOOLCHAIN_FILE;DISABLE_CMAKE_FIND_ROOT_FLAGS;DISABLE_CMAKE_TOOLSET_MODULES;DISABLE_GENERATOR;APPEND_SYSTEM_LINKS"
    ""
    "DISABLE_INHERIT_FLAGS"
    ${ARGN})
  if(NOT project_build_tools_append_cmake_inherit_options_DISABLE_GENERATOR)
...
macro(project_build_tools_append_cmake_host_options OUTVAR)
  cmake_parse_arguments(
    ...
    "DISABLE_C_FLAGS;DISABLE_CXX_FLAGS;DISABLE_ASM_FLAGS;DISABLE_TOOLCHAIN_FILE;DISABLE_CMAKE_TOOLSET_MODULES;DISABLE_GENERATOR;APPEND_SYSTEM_LINKS"
    ""
    "DISABLE_INHERIT_FLAGS"
    ${ARGN})
  if(NOT project_build_tools_append_cmake_inherit_options_DISABLE_GENERATOR)
  ...
  foreach(PROTO_SRC ${ARGN})
    ...
    if(PROTO_SRC_OPTIONS)
      ...
      if(NOT ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_PROTOBUF_CXX_STANDARD
        ...
        if(MSVC)
          string(REGEX REPLACE "/std:c\\+\\+[0-9a-zA-Z_]+" "" PROTO_SRC_OPTIONS "${PROTO_SRC_OPTIONS}")
        endif()
        string(REGEX REPLACE "-std=(c|gnu)\\+\\+[0-9a-zA-Z_]+" "" PROTO_SRC_OPTIONS "${PROTO_SRC_OPTIONS}")
        set(__need_cxx_standard TRUE)
      endif()
    ...
    if(PROTO_SRC_OPTIONS_CHANGED)
      set_source_files_properties(${PROTO_SRC} PROPERTIES COMPILE_OPTIONS "${PROTO_SRC_OPTIONS}")
    endif()
  endforeach()
  if(ATFRAMEWORK_CMAKE_TOOLSET_THIRD_PARTY_PROTOBUF_CXX_STANDARD)
    set_source_files_properties(${ARGN} PROPERTIES CXX_STANDARD
...
function(project_build_tools_generate_load_env_bash OUTPUT_FILE)
  ...
  set(oneValueArgs)
  set(multiValueArgs)
  cmake_parse_arguments(_bash_env "${optionArgs}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
  ...
  if(NOT _bash_env_DISABLE_AR)
    if(COMPILER_OPTION_INHERIT_CMAKE_AR OR ENV{AR})
      file(APPEND "${OUTPUT_FILE}" "export AR=\"${CMAKE_AR}\"${PROJECT_THIRD_PARTY_BUILDTOOLS_BASH_EOL}")
    elseif(COMPILER_OPTION_INHERIT_CMAKE_CXX_COMPILER_AR)
      file(APPEND "${OUTPUT_FILE}" "export AR=\"${CMAKE_AR}\"${PROJECT_THIRD_PARTY_BUILDTOOLS_BASH_EOL}")
    elseif(COMPILER_OPTION_INHERIT_CMAKE_C_COMPILER_AR)
  ...
  if((COMPILER_OPTION_INHERIT_CMAKE_RANLIB
      OR COMPILER_OPTION_INHERIT_CMAKE_C_COMPILER_RANLIB
      OR COMPILER_OPTION_INHERIT_CMAKE_CXX_COMPILER_RANLIB
      OR ENV{RANLIB}
     )
  ...
  if(CMAKE_INSTALL_RPATH)
    ...
    if(NOT _bash_env_DISABLE_LD AND RPATH_LINK_VALUE)
      string(REPLACE "$" "\\$" RPATH_LINK_VALUE "${RPATH_LINK_VALUE}")
...
function(project_build_tool_generate_load_env_powershell OUTPUT_FILE)
  ...
  set(oneValueArgs)
  set(multiValueArgs)
  cmake_parse_arguments(_pwsh_env "${optionArgs}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
