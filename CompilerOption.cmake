...
if(NOT DEFINED __COMPILER_OPTION_LOADED)
  ...
  if(MSVC)
    ...
    option(COMPILER_OPTION_MSVC_ENABLE_LTO_OPTIONS "Use /DEBUG /INCREMENTAL:NO /OPT:REF /OPT:ICF and etc when linking."
           ON)
    ...
    option(COMPILER_OPTION_ENABLE_SPLIT_DEBUGGING_INFORMATION "Split debugging information into standalone files." ON)
  else()
    option(COMPILER_OPTION_ENABLE_SPLIT_DEBUGGING_INFORMATION "Split debugging information into standalone files." OFF)
  endif()
  ...
  if(NOT DEFINED CACHE{CMAKE_INSTALL_RPATH})
    if("$ENV{CMAKE_INSTALL_RPATH}")
      set(COMPILER_OPTION_INHERIT_CMAKE_INSTALL_RPATH "$ENV{CMAKE_INSTALL_RPATH}")
      list(APPEND CMAKE_INSTALL_RPATH "$ENV{CMAKE_INSTALL_RPATH}")
    else()
      if(UNIX AND NOT APPLE)
        list(APPEND CMAKE_INSTALL_RPATH "$ORIGIN" "$ORIGIN/../lib64" "$ORIGIN/../lib")
      ...
      elseif(APPLE)
        # list(APPEND CMAKE_INSTALL_RPATH "@loader_path" "@loader_path/../lib64" "@loader_path/../lib")
        list(APPEND CMAKE_INSTALL_RPATH "@rpath" "@rpath/../lib64" "@rpath/../lib")
        set(COMPILER_OPTION_INHERIT_CMAKE_INSTALL_RPATH "${CMAKE_INSTALL_RPATH}")
        set(CMAKE_INSTALL_RPATH_USE_LINK_PATH YES)
        set(COMPILER_OPTION_INHERIT_CMAKE_INSTALL_RPATH_USE_LINK_PATH YES)
      endif()
  ...
  if(NOT DEFINED CACHE{CMAKE_BUILD_RPATH})
    if("$ENV{CMAKE_BUILD_RPATH}")
      set(COMPILER_OPTION_INHERIT_CMAKE_BUILD_RPATH "$ENV{CMAKE_BUILD_RPATH}")
      list(APPEND CMAKE_BUILD_RPATH "$ENV{CMAKE_BUILD_RPATH}")
    endif()
  endif()
  ...
  if(NOT COMPILER_OPTION_CURRENT_MAX_CXX_STANDARD)
    if(CMAKE_VERSION VERSION_GREATER_EQUAL "3.25.0")
      set(COMPILER_OPTION_CURRENT_MAX_CXX_STANDARD 26)
    elseif(CMAKE_VERSION VERSION_GREATER_EQUAL "3.20.0")
  ...
  set(COMPILER_OPTION_LINK_PATH_FLAG "-L")
  if(ATFRAMEWORK_CMAKE_TOOLSET_TARGET_IS_WINDOWS AND (MSVC OR CMAKE_LINKER_TYPE STREQUAL "MSVC"))
    set(COMPILER_OPTION_LINK_PATH_FLAG "-LIBPATH:")
  endif()
  ...
  macro(add_linker_flags_for_runtime_inherit)
    add_linker_flags_for_runtime(${ARGN})
    foreach(def ${ARGN})
      if(CMAKE_EXE_LINKER_FLAGS)
  ...
  macro(add_linker_flags_for_runtime_unique_inherit)
    foreach(def ${ARGN})
      project_build_tools_append_space_flags_to_var_unique(CMAKE_EXE_LINKER_FLAGS "${def}")
      project_build_tools_append_space_flags_to_var_unique(CMAKE_MODULE_LINKER_FLAGS "${def}")
      project_build_tools_append_space_flags_to_var_unique(CMAKE_SHARED_LINKER_FLAGS "${def}")

      project_build_tools_append_space_flags_to_var_unique(COMPILER_OPTION_INHERIT_CMAKE_EXE_LINKER_FLAGS "${def}")
      project_build_tools_append_space_flags_to_var_unique(COMPILER_OPTION_INHERIT_CMAKE_MODULE_LINKER_FLAGS "${def}")
      project_build_tools_append_space_flags_to_var_unique(COMPILER_OPTION_INHERIT_CMAKE_SHARED_LINKER_FLAGS "${def}")
    endforeach()
  endmacro()
  ...
  macro(add_linker_flags_for_all_inherit)
    add_linker_flags_for_runtime_inherit(${ARGN})
    foreach(def ${ARGN})
      if(CMAKE_STATIC_LINKER_FLAGS)
        set(CMAKE_STATIC_LINKER_FLAGS "${CMAKE_STATIC_LINKER_FLAGS} ${def}")
      else()
      ...
      if(COMPILER_OPTION_INHERIT_CMAKE_STATIC_LINKER_FLAGS)
        set(COMPILER_OPTION_INHERIT_CMAKE_STATIC_LINKER_FLAGS "${CMAKE_STATIC_LINKER_FLAGS} ${def}")
      else()
        set(COMPILER_OPTION_INHERIT_CMAKE_STATIC_LINKER_FLAGS "${def}")
      endif()
    endforeach()
  endmacro()
  ...
  macro(add_linker_flags_for_all_unique_inherit)
    foreach(def ${ARGN})
      project_build_tools_append_space_flags_to_var_unique(CMAKE_EXE_LINKER_FLAGS "${def}")
      project_build_tools_append_space_flags_to_var_unique(CMAKE_MODULE_LINKER_FLAGS "${def}")
      project_build_tools_append_space_flags_to_var_unique(CMAKE_SHARED_LINKER_FLAGS "${def}")
      ...
      project_build_tools_append_space_flags_to_var_unique(COMPILER_OPTION_INHERIT_CMAKE_MODULE_LINKER_FLAGS "${def}")
      project_build_tools_append_space_flags_to_var_unique(COMPILER_OPTION_INHERIT_CMAKE_SHARED_LINKER_FLAGS "${def}")
      project_build_tools_append_space_flags_to_var_unique(COMPILER_OPTION_INHERIT_CMAKE_STATIC_LINKER_FLAGS "${def}")
    endforeach()
  endmacro()
  ...
  macro(try_set_linker)
    ...
    if(CMAKE_CXX_COMPILER_ID MATCHES "AppleClang|Clang|GNU")
      ...
      foreach(try_set_linker_TRY_LINKER ${try_set_linker_LINKER})
        ...
        cmake_push_check_state()
        list(APPEND CMAKE_REQUIRED_LINK_OPTIONS "${try_set_linker_LINK_OPTION}")
        ...
        if(try_set_linker_test_compiling)
          ...
          set(ATFRAMEWORK_CMAKE_TOOLSET_LINKER_OPTIONS "${try_set_linker_LINK_OPTION}")
  ...
  elseif(MSVC)
    ...
    if(NOT VCPKG_TOOLCHAIN)
      ...
      add_compiler_flags_to_inherit_var_unique(CMAKE_CXX_FLAGS "/utf-8")
      add_compiler_flags_to_inherit_var_unique(CMAKE_C_FLAGS "/utf-8")
      add_compiler_flags_to_inherit_var_unique(CMAKE_RC_FLAGS "-c65001")
    endif()
  ...
  if(MSVC)
    ...
    if(NOT VCPKG_TOOLCHAIN)
      add_compiler_flags_to_inherit_var_unique(CMAKE_CXX_FLAGS_DEBUG /Od /D_DEBUG /Ob0)
      add_compiler_flags_to_inherit_var_unique(CMAKE_C_FLAGS_DEBUG /Od /D_DEBUG /Ob0)
      add_compiler_flags_to_inherit_var_unique(CMAKE_CXX_FLAGS_RELEASE /O2 /Oi /DNDEBUG)
      add_compiler_flags_to_inherit_var_unique(CMAKE_C_FLAGS_RELEASE /O2 /Oi /DNDEBUG)
      add_compiler_flags_to_inherit_var_unique(CMAKE_CXX_FLAGS_RELWITHDEBINFO /O2 /Oi /DNDEBUG)
      add_compiler_flags_to_inherit_var_unique(CMAKE_C_FLAGS_RELWITHDEBINFO /O2 /Oi /DNDEBUG)
      add_compiler_flags_to_inherit_var_unique(CMAKE_CXX_FLAGS_MINSIZEREL /Ox /DNDEBUG)
      add_compiler_flags_to_inherit_var_unique(CMAKE_C_FLAGS_MINSIZEREL /Ox /DNDEBUG)
      if(NOT CMAKE_MSVC_DEBUG_INFORMATION_FORMAT AND NOT CMAKE_CXX_FLAGS MATCHES "/Z7|/Zi|/ZI")
        if(NOT COMPILER_OPTION_ENABLE_SPLIT_DEBUGGING_INFORMATION)
          if(CMAKE_VERSION VERSION_GREATER_EQUAL "3.25.0")
            set(CMAKE_MSVC_DEBUG_INFORMATION_FORMAT "Embedded")
          else()
            add_compiler_flags_to_inherit_var_unique(CMAKE_CXX_FLAGS_DEBUG /Z7)
            add_compiler_flags_to_inherit_var_unique(CMAKE_C_FLAGS_DEBUG /Z7)
            add_compiler_flags_to_inherit_var_unique(CMAKE_CXX_FLAGS_RELEASE /Z7)
            add_compiler_flags_to_inherit_var_unique(CMAKE_C_FLAGS_RELEASE /Z7)
            add_compiler_flags_to_inherit_var_unique(CMAKE_CXX_FLAGS_RELWITHDEBINFO /Z7)
            add_compiler_flags_to_inherit_var_unique(CMAKE_C_FLAGS_RELWITHDEBINFO /Z7)
            add_compiler_flags_to_inherit_var_unique(CMAKE_CXX_FLAGS_MINSIZEREL /Z7)
            add_compiler_flags_to_inherit_var_unique(CMAKE_C_FLAGS_MINSIZEREL /Z7)
          endif()
        else()
          if(CMAKE_VERSION VERSION_GREATER_EQUAL "3.25.0")
            set(CMAKE_MSVC_DEBUG_INFORMATION_FORMAT "ProgramDatabase")
            # The default value is /Zi, so do not need to add any additional flags
          endif()
        endif()
      endif()
      if(COMPILER_OPTION_MSVC_ENABLE_LTO_OPTIONS)
      ...
      if(COMPILER_OPTION_MSVC_ENABLE_LTO_OPTIONS)
        ...
        add_compiler_flags_to_inherit_var_unique(CMAKE_SHARED_LINKER_FLAGS_RELEASE " /nologo ")
        add_compiler_flags_to_inherit_var_unique(CMAKE_EXE_LINKER_FLAGS_RELEASE " /nologo ")
      endif()
    ...
    if(CMAKE_HOST_SYSTEM_NAME STREQUAL "Darwin")
      ...
      if(CMAKE_LIBTOOL)
        ...
        if("${LIBTOOL_V_OUTPUT}" MATCHES ".*cctools-([0-9.]+).*")
          set(CMAKE_LIBTOOL_IS_CCTOOLS TRUE)
          ...
          set(CMAKE_LIBTOOL_IS_CCTOOLS FALSE)
        endif()
    ...
    if(COMPILER_OPTION_ENABLE_SPLIT_DEBUGGING_INFORMATION)
      if(${CMAKE_CXX_COMPILER_ID} STREQUAL "GNU")
        add_compiler_flags_to_inherit_var_unique(CMAKE_CXX_FLAGS "-gsplit-dwarf")
        # add_linker_flags_for_runtime_unique("-Wl,--gdb-index")
      elseif(CMAKE_CXX_COMPILER_ID MATCHES "AppleClang|Clang")
        if(COMPILER_OPTION_ENABLE_SPLIT_DEBUGGING_INFORMATION STREQUAL "single")
          add_compiler_flags_to_inherit_var_unique(CMAKE_CXX_FLAGS "-gsplit-dwarf=single")
        else()
          add_compiler_flags_to_inherit_var_unique(CMAKE_CXX_FLAGS "-gsplit-dwarf=split")
        endif()
        # add_linker_flags_for_runtime_unique("-Wl,--gdb-index")
      endif()
    endif()
  ...
  if(NOT DEFINED COMPILER_OPTIONS_TEST_STD_COROUTINE)
    ...
    if(COMPILER_OPTIONS_TEST_STD_COROUTINE)
      ...
      if(NOT MSVC)
        # Try add coroutine
        set(CMAKE_REQUIRED_FLAGS "${CMAKE_REQUIRED_FLAGS} -fcoroutines")
        check_cxx_source_compiles(
          "#include <coroutine>
         int main() {
           return std::suspend_always().await_ready()? 0: 1;
        ...
        elseif()
          set(CMAKE_REQUIRED_FLAGS "${CMAKE_REQUIRED_FLAGS} -fcoroutines-ts")
          check_cxx_source_compiles(
        ...
        if(NOT COMPILER_OPTIONS_TEST_STD_COROUTINE)
          set(CMAKE_REQUIRED_FLAGS "${CMAKE_REQUIRED_FLAGS} /await")
          check_cxx_source_compiles(
            "#include <coroutine>
              int main() {
                  return std::suspend_always().await_ready()? 0: 1;
